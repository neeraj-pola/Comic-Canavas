...
Write-Bold "Done. Start the app:"
Write-Info "uv run honcho start -f Procfile.full   # api + worker + scheduler + web, one command, one window"
Write-Host ""
Write-Info "Then open http://localhost:3000 (honcho's web.1 log line shows the real URL if 3000 is busy)."
Write-Host ""
Write-Info "Prefer two separate windows instead? 'uv run honcho start -f Procfile.dev' (backend only)"
Write-Info "and, separately, 'cd apps/web; pnpm dev' (frontend only) still work exactly as before."
